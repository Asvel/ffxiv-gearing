...
const ProductionMateriaCalculationPanel = mobxReact.observer(() => {
  ...
  const [targets, setTargets] = React.useState<Record<string, string>>(() => Object.fromEntries(
    stats.map(stat => [stat, String(store.equippedStatsWithoutFood[stat] ?? store.productionMateriaBaseStats[stat] ?? 0)]),
  ));
  ...
  const invalidStats = new Set(stats.filter(stat =>
    targets[stat].trim() === '' || !Number.isInteger(parsed[stat]) ||
      parsed[stat]! < (store.productionMateriaBaseStats[stat] ?? 0),
  ));
  const tooHighStats = new Set(stats.filter(stat =>
    Number.isInteger(parsed[stat]) && parsed[stat]! > (store.productionMateriaMaximumStats[stat] ?? 0),
  ));
  React.useEffect(() => () => {
    requestId.current++;
  ...
  return (
    <div className="production-materia-optimization card">
      ...
      <div className="production-materia-optimization_targets">
        {stats.map(stat => {
          ...
          return (
            <label className="production-materia-optimization_target" key={stat}>
              ...
              <TextField
                className={classNames('production-materia-optimization_input mdc-text-field--compact', invalid && '-invalid')}
                type="number"
                min={minimum}
                step="1"
                value={targets[stat]}
        ...
        <Button
          disabled={invalidStats.size > 0 || tooHighStats.size > 0 || calculating}
          onClick={calculate}
        >
          {calculating ? '计算中' : '计算'}
        </Button>
...
const ProductionMateriaResultView = mobxReact.observer<{
  ...
  const materiaCount = result.plan.reduce((sum, gear) =>
    sum + gear.materias.filter(materia => materia.stat !== undefined).length, 0);
  return (
    <div className="production-materia-optimization_result">
      <table className="production-materia-optimization_summary table">
        <tbody>
          <tr>
            {stats.map(stat => <th key={stat}>{G.statNames[stat]}</th>)}
          </tr>
          <tr>
            {stats.map(stat => <td key={stat}>{result.stats[stat]}</td>)}
          </tr>
        </tbody>
      </table>
          ...
          {result.plan.flatMap(gearPlan => {
            ...
            return [(
              <tr key={gearPlan.gearId}>
                ...
                <td>{materias.map(materia => G.getMateriaName(
                  materia.stat!,
                  materia.grade!,
