...
export const useLinearProgressFoundation = (props: LinearProgressProps) => {
  ...
  useEffect(() => {
    ...
    if (isDeterminate !== determinate) {
      ...
      setDeterminate(isDeterminate);  // eslint-disable-line react-hooks/set-state-in-effect
    }
  ...
  useEffect(() => {
    ...
    if (props.closed) {
      const timeout = setTimeout(() => foundation.handleTransitionEnd(), 1000);
